...
check_debian() {
	if [ -n "$(cat /etc/*-release | grep -i 'ID=debian')" ] ; then
		...
		echo "license_key: insert license here" | tee -a /etc/newrelic-infra.yml
...
check_amazon() {
	if [ -n "$(cat /etc/*-release | grep -i 'ID="amzn"')" ] ; then
		echo "license_key: insert license here" | sudo tee -a /etc/newrelic-infra.yml
...
check_centos7() {
	if [ -n "$(cat /etc/*-release | grep -i 'ID="centos"')" ] ; then
		echo "license_key: 26952a025640e073b97cd75045963b06014823f8" | sudo tee -a /etc/newrelic-infra.yml
